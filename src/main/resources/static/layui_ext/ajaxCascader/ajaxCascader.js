...
layui.define(["jquery"], function (exports) {
	...
	Private.prototype.init = function (options) {
		...
		var clearButtonDom = '';
		var clearInput = '';
		// 宽度、高度过滤【思路来自于github@alasq】
		if (isNumber(param.width)) {
			param.width += 'px';
		}
		if (isNumber(param.height)) {
			param.height += 'px';
		}
		...
		if (param.clear) {
			...
			clearButtonDom = "<i class=\"layui-icon layui-icon-close cascader-clear\" style=\"top:50%;\"></i>";
		}
		// 渲染主dom
		store.cascaderDom.after("\n\t\t\t<div class=\"cascader-all " + param.className + " " + clearInput + "\" style=\"width:" + this.param.width + ";\">\n\t\t\t\t<input type=\"text\" class=\"layui-input cascader-input\" placeholder=\"" + param.placeholder + "\" readonly style=\"width:" + this.param.width + ";height:" + this.param.height + ";\">\n\t\t\t\t<i class=\"layui-icon layui-icon-down cascader-i\" style=\"top:50%;\"></i>\n\t\t\t\t" + clearButtonDom + "\n\t\t\t\t<div class=\"cascader-model " + phoneName + "\" style=\"z-index:" + this.store.zIndex + ";display:flex;\">\n\t\t\t\t</div>\n\t\t\t</div>\n \t\t");
